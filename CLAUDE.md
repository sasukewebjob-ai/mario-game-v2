...
| 5 | 4 | 効く | 2.0 | 6.5 | -4.0 | 85 | 215 |
| 6 | 4 | 効く | 2.2 | 7.0 | -4.0 | 75 | 205 |
| 7 | 5 | 効く | 2.4 | 7.5 | -4.5 | 65 | 195 |
| 8 | 5 | 効く | 2.6 | 8.0 | -5.0 | 60 | 175 |
...
| 8 | 8-1 | `levels/level8-1.js` | ✅ 飛行船・強制スクロール(0.8)・G.airshipMode・キャノン×10・ワープ→ゴール |
| 8 | 8-2 | `levels/level8-2.js` | ✅ 飛行船・強制スクロール(1.0)・G.airshipMode・キャノン×11・ハンマーブロス×2・ワープ→ゴール |
...
### bgTheme: 'airship' / G.airshipMode
暗黒の夜空 + 星 + 嵐雲 + 稲光フラッシュ + 艦隊シルエット + サーチライト。`bgmTheme:'castle'` を使用。
`G.airshipMode=true` を設定すると `drawTile()` の ground/brick ブロックが鋼鉄甲板デザインに変わる。
リセット時（startFromStage/restartCurrentLevel/チェックポイント復帰）に自動で `false` に戻る。
