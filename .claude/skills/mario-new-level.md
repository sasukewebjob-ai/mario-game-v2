...
以下の3つは**全ステージ共通の必須作業**です。ユーザーから指示がなくても必ず実施してください：
...
**よくある重複パターン（必ず確認）：**
```javascript
// NG: addRow と push が同じ x,y
addRow(1300, H-8*TILE, 2, 'q', true);  // x=1300 と 1332
platforms.push({x:1300, y:H-8*TILE, ..., hasMush:true});  // ← x=1300 が重複！

// OK: addRow の範囲から特殊ブロックを除外
addRow(1332, H-8*TILE, 1, 'q');        // x=1332 だけ
platforms.push({x:1300, y:H-8*TILE, ..., hasMush:true});  // x=1300 は push のみ
```
...
- [ ] **addRow と platforms.push の座標重複がないか確認・修正したか**
