...
export function buildLevel_6_3(){
  ...
  pipes.push({x:6000,y:0,w:TILE*2,h:3*TILE,bounceOffset:0,isWarp:false,ceiling:true});
  piranhas.push({x:4524,baseY:3*TILE,y:3*TILE,w:16,h:TILE,phase:piranhas.length*0.7,alive:true,maxUp:TILE*1.5,ceiling:true});
  // 追いかけ壁（氷床スライド + 壁で難度最大）
