...
export function buildLevel_7_3(){
  ...
  // 地面（玉座の深淵×3）
  const gaps=[{s:2000,e:2300},{s:3700,e:4000},{s:5500,e:5800}];
  ...
  // Z3: 後半
  addRow(4060,H-5*TILE, 2,'brick'); // 4060,4092 → 末端4124（ドッスン4150-4214の下にブロック置かない）
  ...
  // Z3
  platforms.push({x:4224,y:H-5*TILE, w:TILE,h:TILE,type:'question',hit:false,hasMush:true,bounceOffset:0}); // 4224>4214(ドッスン右端) ✓
  ...
  movingPlats.push({x:3760,y:H-4*TILE,w:TILE*2,h:12,type:'h',ox:3760,range:85, spd:1.8,prevX:3760});
  ...
  for(let fx=3710;fx<3990;fx+=38) lavaFlames.push({x:fx,y:H-TILE,w:12,maxH:54+((fx-3710)%3)*14,curH:0,phase:Math.floor((fx-3710)/38)*9,period:78+fx%20});
  ...
  // ノコノコ（城内巡回）
  [550, 900, 1400, 1900, 2500, 3000, 4100, 4700, 5100, 5900, 6200].forEach(ex=>{
  ...
  // ── コイン ──
  // ① ギャップアーチ（各10枚 × 3 = 30枚）
  [{s:2000,e:2300},{s:3700,e:4000},{s:5500,e:5800}].forEach(({s,e})=>{
  ...
  [[100,H-7*TILE,4],[400,H-9*TILE,3],[1050,H-7*TILE,5],[2350,H-7*TILE,4],
   [4060,H-7*TILE,4],[5860,H-7*TILE,4]].forEach(([cx,cy,n])=>{
  ...
  for(let j=0;j<18;j++) coinItems.push({x:4050+j*82, y:H-3*TILE,collected:false}); // Z3
  ...
  // ピーチ（アリーナ右端）
  peach.alive=true;
  peach.x=7500;peach.y=H-TILE-peach.h;
